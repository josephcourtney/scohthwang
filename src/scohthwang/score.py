...
class PairCostFn(Protocol):
    ...
    def __call__(self, left: Any, right: Any) -> float:  # ruff: ignore[any-type] - protocol must remain generic across caller-defined element types.
...
class ConstraintFn(Protocol):
    ...
    def __call__(self, left: Any, right: Any) -> bool:  # ruff: ignore[any-type] - protocol must remain generic across caller-defined element types.
...
def make_pair_cost_fn(config: PairCostConfig) -> PairCostFn:
    ...
    def pair_cost(left: Any, right: Any) -> float:  # ruff: ignore[any-type] - closure implements PairCostFn for arbitrary caller element shapes.
...
def make_nested_cost_fn(
    ...
    def nested_cost(left: Any, right: Any) -> float:  # ruff: ignore[any-type] - nested matcher accepts arbitrary caller element/container types.
